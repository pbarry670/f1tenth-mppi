...
class MPPIControl(Node):
    ...
    def __init__(self):
        ...
        self.V_DESIRED = 0.5 #desired car velocity, m/s
        self.VEL_COST_SCALER = 100.0 #scalar parameter to tune cost associated with deviating from velocity cost
        ...
        self.POS_COST_SCALER = 500.0 #scalar parameter to tune cost associated with deviation from centerline
    ...
    def compute_cost(self, x_cuda, u_steer, u_throttle):
        ...
        ctrl_cost = self.CTRL_INPUT_COST_SCALER*(torch.square(u_steer - self.prev_steer_cmd) + torch.square(u_throttle - self.prev_accel_cmd))
...
def main(args=None):
    ...
    mppi.u_throttle = mppi.generate_throttle_control_sequence(1.0,0.5) #mean of 0.5, variance of 0.5
    ...
    for i in range(100):
        ...
        if (steer_command_pub >= 0.5):
            steer_command_pub = steer_command_pub/(1 + steer_command_pub)
        if (steer_command_pub <= -0.5):
            steer_command_pub = steer_command_pub/(1 - steer_command_pub)
