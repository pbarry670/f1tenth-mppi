...
class MPPIControl(Node):
    ...
    def __init__(self):
        ...
        super().__init__("carless_mppi") #Name of node
        ...
        self.NUM_TRAJECTORIES = 100 #Number of trajectories
        ...
        self.trueDT = 0.2 #Time step between control inputs in the real world (assumes operation at ~5 Hz)
        ...
        self.RACELINE_WIDTH = 1.0 #distance from track boundary to track boundary, m
        self.NUM_RACELINE_PTS = 30.0 #Number of points on discretized centerline
        self.POS_COST_SCALER = 5000.0 #scalar parameter to tune cost associated with deviation from centerline

        self.HEADING_COST_SCALER = 5.0 #scalar parameter to tune cost associated with devition from ideal heading angle

        self.CTRL_INPUT_COST_SCALER = 0.4 #scalar parameter to tune cost associated with magnitude of control inputs
    ...
    def state_forward(self, x_vec, u_steer_vel, u_throttle): #Dummy function for testing
        ...
        x_new.append(x_vec[3] + self.throttle_to_accel_mapping(u_throttle)*self.trueDT)
    ...
    def compute_cost(self, x_cuda, u_steer_vel, u_throttle):
        ...
        ctrl_cost = self.CTRL_INPUT_COST_SCALER*(torch.square(u_steer_vel - self.prev_steer_vel_cmd) + torch.square(u_throttle - self.prev_accel_cmd))
        vel_cost = torch.square(self.V_DESIRED - x_cuda[3,:])

        print(f"Position cost: {pos_cost}")
        print(f"Velocity cost: {vel_cost}")
        print(f"Heading cost: {heading_cost}")
        print(f"Control cost: {ctrl_cost}")

        cost = self.VEL_COST_SCALER*vel_cost + self.POS_COST_SCALER*pos_cost + self.HEADING_COST_SCALER*heading_cost + ctrl_cost
        return cost
    ...
    def compute_position_and_heading_cost(self, x_cuda):
        ...
        x_pos = (x_cuda[0,:]).cpu()
        y_loc = interpolate.splev(x_pos, self.curve) #Obtain y location based on spline interpolation
        y_loc = (torch.tensor(y_loc)).cuda()
        ...
        interpolated_heading = (torch.tensor(interpolated_heading)).cuda()
    ...
    def compute_centerline(self):
        ...
        self.heading_curve = interpolate.splrep(centerline_x_short, self.optimal_heading)
...
def main(args=None):
    ...
    mppi.u_steering_vel = mppi.generate_steering_control_sequence(0, 2.0) #mean of 0, variance of 1.5 rad/s
    mppi.u_throttle = mppi.generate_throttle_control_sequence(1,1) #mean of 1, variance of 1
    ...
    u_steering_vel_cuda = mppi.u_steering_vel.cuda()
    ...
    plotted_x = np.zeros((mppi.HORIZON_LENGTH, mppi.NUM_TRAJECTORIES))
    plotted_y = np.zeros((mppi.HORIZON_LENGTH, mppi.NUM_TRAJECTORIES))
    ...
    for i in range(mppi.HORIZON_LENGTH - 1): #Loop through the dynamics with each sequential control input over the horizon
        x_new = mppi.forward_dynamics(x0_cuda, u_steering_vel_cuda[i,:], u_throttle_cuda[i,:])

        x_vectors = (x_new.cpu()).numpy() #Results in numpy array x_vectors
        x_coords = x_vectors[0,:]
        y_coords = x_vectors[1,:]

        plotted_x[i,:] = x_coords
        plotted_y[i,:] = y_coords

        cost_cuda = cost_cuda + mppi.compute_cost(x_new, u_steering_vel_cuda[i,:], u_throttle_cuda[i,:]) #Compute cost at each step forward in time
    
    mppi.cost = cost_cuda.cpu()
    index_optimal_trajectory = torch.argmin(mppi.cost) #Select trajectory with the minimal cost
    steer_vel_command = u_steering_vel_cuda[0,index_optimal_trajectory]
    ...
    mppi.prev_steer_vel_cmd = steer_vel_command_pub
    mppi.prev_accel_cmd = throttle_command_pub

    print(f"Steering Velocity Command: {steer_vel_command_pub}")
    print(f"Acceleration Command: {throttle_command_pub}")
    #print(plotted_x[:,index_optimal_trajectory])
    #print(plotted_y[:, index_optimal_trajectory])

    plt.plot(mppi.centerline[:,0], mppi.centerline[:,1], color = 'blue')
    ...
    for i in range(mppi.NUM_TRAJECTORIES - 1):
        plt.plot(plotted_x[0:-2,i], plotted_y[0:-2,i], color = 'red')
        plt.pause(0.01)
    plt.plot(plotted_x[0:-2, index_optimal_trajectory], plotted_y[0:-2, index_optimal_trajectory], color = 'green')
    plt.show()
    ...
    x_vec = [0,0,0,0,0]
    x_vec = mppi.state_forward(x_vec, steer_vel_command_pub, throttle_command_pub) #Testing dummy for state estimate
    ...
    for i in range(100):
        ...
        x_cuda = mppi.x.cuda()
        mppi.u_steering_vel = mppi.generate_steering_velocity_control_sequence(steer_vel_command_pub, 3.0)
        ...
        for i in range(mppi.HORIZON_LENGTH - 1):
            x_new = mppi.forward_dynamics(x_cuda, u_steering_vel_cuda[i,:], u_throttle_cuda[i,:])
            ...
            cost_cuda = cost_cuda + mppi.compute_cost(x_new, u_steering_vel_cuda[i,:], u_throttle_cuda[i,:])

        mppi.cost = cost_cuda.cpu()
        index_optimal_trajectory = torch.argmin(mppi.cost)
        steer_vel_command = u_steering_vel_cuda[0, index_optimal_trajectory].cpu()
        throttle_command = u_throttle_cuda[0, index_optimal_trajectory].cpu()

        steer_vel_command_pub = steer_vel_command.item() #Values to be published to Ackermann
        throttle_command_pub = throttle_command.item()

        mppi.set_steer(steer_vel_command_pub)
        mppi.set_accel(throttle_command_pub)
    ...
    print(f"Time per control loop (seconds): {sum(timeVec)/len(timeVec)}")
